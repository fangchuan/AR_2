...
#define MPU_INT_PORT_CLK  RCC_APB2Periph_GPIOA
#define MPU_INT_PIN       GPIO_Pin_15
#define MPU_INT_PORT      GPIOA

//供外部调用的API
extern void MPU6050_initialize(void); //初始化
extern uint8_t MPU6050_testConnection(void); //检测MPU6050是否存在
extern u8 MPU6050_is_DRY(void);
extern void MPU6050_getMotion6(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz);
extern void MPU6050_getlastMotion6(int16_t* ax, int16_t* ay, 
																	 int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz);
extern void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az);//ahrs结算
extern void Get_Attitude(void);
//#define kalman 1

#ifdef kalman
float Kalman_Filter(float angle_m,float gyro_m);	  //kalman运算函数
